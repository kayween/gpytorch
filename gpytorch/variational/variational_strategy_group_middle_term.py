...
class VariationalStrategyGroupMiddleTermSinglePrecision(VariationalStrategy):
    r"""
    An ultra-light-weight variational strategy that perform linear algebra operations on torch tensors.

    This class groups the middle term `K_ZZ^{-1/2} (S - I) K_ZZ^{-1/2}` and does everything in single precision (except the Cholesky decomposition).
    """
    ...
    def forward(
        ...
        **kwargs,
        ...
        L = L.type(induc_induc_covar.dtype)
        # Compute the mean of q(f)
        # k_XZ K_ZZ^{-1/2} (m - K_ZZ^{-1/2} \mu_Z) + \mu_X
        vec = torch.linalg.solve_triangular(L.mT, inducing_values.unsqueeze(-1), upper=True, left=True)
        ...
        right_term = middle_term @ induc_data_covar
...
class VariationalStrategyGroupMiddleTermDoublePrecision(VariationalStrategy):
    r"""
    An ultra-light-weight variational strategy that perform linear algebra operations on torch tensors.
